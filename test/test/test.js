...
describe('createChild Pico for testing', function() {
  this.slow(10000);// this might take some time.
  var first_response;
  var second_response;
  var new_pico;
  ...
  it('list should differ by one if new pico created.', function() {
    ...
    it('list installed rulesets with wrangler.dev',function(done) {
     this.retries(2);
      ...
      assert.notInclude(response.rids,'b507199x5.dev','b507199x5.dev(wrangler.prod) should be installed in child pico');
      assert.notInclude(response.rids,'b507199x1.dev','b507199x1.dev(bootstrapping.prod) should be installed in child pico');
       if (err) {
